...
do so your job will be terminated 120 hours after it starts. For details
refer to
<http://projects.iq.harvard.edu/rce/book/extending-rce-powered-application>.
...
intensive tasks to run faster. **If you are not interested in running
parallel processes in interactive jobs feel free to skip to the *next
section**.
...
We have `len(textFiles)`{.python .rundoc-block rundoc-language="python"}
{{{results(`2080`)}}} text files to process. We can do this using a
single core:
...
. It takes around `print(stop - start)`{.python .rundoc-block
rundoc-language="python"} {{{results(`1.2721843719482422`)}}} seconds to
...
which reduces the time to `print(end - start)`{.python .rundoc-block
rundoc-language="python"} {{{results(`0.33490753173828125`)}}} seconds.
